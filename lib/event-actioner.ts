...
export class EventActioner extends Construct {
  constructor(scope: Construct, id: string) {
    ...
    const lambdaFn = new Function(this, "eventActioner", {
      functionName: `${process.env.ENV}EventActionerStack`,
      runtime: Runtime.NODEJS_18_X,
      code: Code.fromAsset("../event-actioner/."),
      ...
      environment: {
        ...
        EVENT_WEBHOOK_QUEUE_URL: `https://sqs.${process.env.CDK_DEFAULT_REGION}.amazonaws.com/${process.env.CDK_DEFAULT_ACCOUNT}/${process.env.ENV}Event_Payload_Post_Actioner_Queue.fifo`,
        EVENT_NOTIFICATION_QUEUE_URL: `https://sqs.${process.env.CDK_DEFAULT_REGION}..amazonaws.com/${process.env.CDK_DEFAULT_ACCOUNT}/${process.env.ENV}Event_Notification_Queue.fifo`,
        EVENT_DOCUMENTS_QUEUE_URL: `https://sqs.${process.env.CDK_DEFAULT_REGION}..amazonaws.com/${process.env.CDK_DEFAULT_ACCOUNT}/${process.env.ENV}campaign-messages-queue`,
      ...
      securityGroups: [
        SecurityGroup.fromLookupById(
          this,
          `${process.env.ENV}EventPayloadActionerSecurityGroupId`,
    ...
    //Dead-Letter-Queue
    const eventActionerDeadLetterQueue = new Queue(
      this,
      `${process.env.ENV}event-payload-actioner-dlq.fifo`,
      {
        fifo: true,
    ...
    // Add tags to the DLQ
    cdk.Tags.of(eventActionerDeadLetterQueue).add(
      "product",
      this.node.tryGetContext("product")
    );
    cdk.Tags.of(eventActionerDeadLetterQueue).add(
    ...
    // Create an SQS queue
    const eventActionerQueue = new Queue(
      this,
      `${process.env.ENV}event-payload-actioner.fifo`,
      {
        ...
        deadLetterQueue: {
          queue: eventActionerDeadLetterQueue,
          maxReceiveCount: 2,
        },
      }
    );
    ...
    cdk.Tags.of(eventActionerQueue).add(
      "product",
      this.node.tryGetContext("product")
    );
    cdk.Tags.of(eventActionerQueue).add(
    ...
    lambdaFn.addEventSource(
      new SqsEventSource(eventActionerQueue, {
        batchSize: 5,
        enabled: true,
      })
    );
